...
- `/tools` treats names, descriptions, origins, and parameter schemas as untrusted. Catalog size, strings, recursive depth, node/property/array counts, and each schema's serialized UTF-8 byte size are bounded in main and independently enforced again by the shared parser in preload. Sanitization reads guarded own data descriptors only, emits null-prototype records, drops colliding normalized keys, replaces accessors/cycles/reflection failures with a sentinel, and never calls `toJSON`. Proxy reflection traps cannot be prevented from running in JavaScript, but a thrown trap fails that subtree closed. SDK tool objects, implementations, output handlers, and filesystem authority never cross IPC.
- Live/restored tool and shell output uses a 65,536 UTF-16-character limit (not a byte or KiB claim); the truncation marker is included inside that limit. Restored entry/tree DTOs omit raw SDK entries, cap message fields and collection depth/count, recursively bound tool arguments/details, and keep each serialized entry/tree collection below a 1 MiB UTF-8 byte budget. Direct-shell results are strictly parsed in both main and preload.
- `/reload` uses public `AgentSession.reload()` in place. The runtime pool serializes it with reload, stop, restart, new-session, switch, and replacement transitions, verifies the exact runtime/session identity after the await, and rejects reload while that target has active agent work. Only bounded category counts and diagnostics cross IPC; resource contents and extension implementations remain main-owned. Extensions remain disabled by policy after reload.

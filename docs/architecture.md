...
Runtime session files remain main/Pi-owned. The adapter metadata-preflights a finite set, delegates JSONL interpretation to public `SessionManager` listing/opening APIs, and emits tagged records with unique opaque catalog IDs—never file paths or Pi objects. Main maps remembered legacy references to their persisted runtime/path and redacts those paths from renderer settings; React keys use catalog identity while activity uses runtime session identity. Native and remembered conflicts are deduplicated before selection. Import and export paths come only from Electron dialogs. Import uses no-follow staging, public SDK validation, exclusive destination creation, and public runtime switching; clone creates a known public-SDK branch artifact before switching. Replacement failure stops/removes the invalid owner, publishes failed recovery state, and cleans known app-created artifacts. Working directories remain GUI-owned settings, and the rail groups bounded metadata without filesystem access.

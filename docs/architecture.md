...
Third-party extension metadata is discovered without evaluating source. User and trusted-project enablement requests persist in a separate `0600` policy file, default disabled, but every discovered extension remains `execution: blocked`. An app-owned Electron utility process provides bounded lifecycle, crash-restart, timeout, and cleanup isolation. Its protocol intentionally accepts no extension path or load command.

A main-owned UI broker defines strict select/confirm/input/editor dialogs, cancellation/timeouts, notifications, status/sidebar sections, inert custom messages, and portable tool-render DTOs. Renderer responses carry only request IDs and bounded values. Malformed or oversized utility messages terminate the host; crashes close pending dialogs and restart at most three times. Terminal-only widgets, overlays, editor replacement, custom footers, and key interception have explicit no-op metadata fallbacks rather than renderer execution.

Pi 0.84.2's public extension API binds factories, event handlers, tools, and `ExtensionContext` directly to in-process `AgentSession`, `SessionManager`, model, and TUI objects. It exposes no serializable remote-extension adapter capable of preserving tool/event/session semantics across a utility boundary. Therefore production continues to create Pi services with `noExtensions: true`, `extensionDialogs` remains false, and no third-party code runs in Electron main, utility, preload, or renderer. The isolation/UI protocol is ready for an upstream remote host adapter without weakening the boundary.

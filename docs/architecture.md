...
Production and tests now share the same application-domain `AgentRuntime` boundary. Production constructs `EmbeddedPiRuntime`; deterministic tests inject `FakePiRuntime` in-process. There is no RPC subprocess, launch specification, binary discovery, strict JSONL transport, runtime selector, or capability table keyed by runtime. Third-party Pi extensions remain disabled until extension isolation and desktop UI contracts are integrated.

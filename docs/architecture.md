...
Runtime session files remain Pi-owned. The embedded adapter uses public `SessionManager.list()`/`listAll()` metadata and emits a bounded catalog containing opaque ID, bounded name/first-message preview, cwd, timestamps, count, and parent ID—never a session-file path or Pi object. App-owned recents remain fallback UI metadata but cannot override a matching Pi record. Import paths and HTML/JSONL export destinations come only from Electron native dialogs. JSONL export copies the Pi-owned file without interpreting it; import, clone, tree navigation, and labels use public SDK operations. Working directories remain GUI-owned settings. The renderer requests Electron's native folder chooser through validated IPC, and the rail groups bounded catalog metadata without filesystem access.

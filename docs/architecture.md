...
The embedded adapter uses public `AgentSession.systemPrompt`, `getAllTools()`, `getActiveToolNames()`, and `reload()` APIs. `/reload` invokes `AgentSession.reload()` in place, preserving the session while Pi performs its extension/resource shutdown, rediscovery, and startup lifecycle. Reload synchronously reserves its exact manager before joining the runtime-pool transition queue. Direct prompts route through that same pool work gate, and prompt-queue scheduling (including settle and background-session paths) defers until the reservation is released; an already-claimed work start makes reload reject. This avoids a transition-queue deadlock while preserving queue ordering and reload serialization with session lifecycle changes. Success and failure release the reservation, and exact session/runtime identity is checked after Pi returns. The adapter returns bounded before/after counts for skills, prompts, themes, context files, extensions, and tools plus bounded diagnostics. Renderer state then refreshes from authoritative SDK-backed operations. Third-party extensions remain disabled, so reload does not weaken the existing extension trust decision.

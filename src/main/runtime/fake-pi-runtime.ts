...
export class FakePiRuntime implements AgentRuntime {
  ...
  private state: AgentState = freshState();
  private messages: AgentMessage[] = [];
  private stopped = true;
  private held: { resolve: () => void } | null = null;

  constructor(private readonly sink: RuntimeSink) {}

  async start(config: RuntimeLaunchConfig): Promise<void> {
    this.stopped = false;
    this.state = {
      ...freshState(),
      sessionFile: `${config.cwd}/.pi/fake-${this.state.sessionId}.jsonl`,
    };
  ...
  async prompt(input: PromptInput): Promise<void> {
    ...
    if (input.text.includes('use a tool') || input.text.includes('reason about')) {
  ...
  getTree(): Promise<TreeSnapshot> {
    return Promise.resolve({ tree: [], leafId: null });
  }

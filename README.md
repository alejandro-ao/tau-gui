...
- Manage working directories in the collapsible left rail, with stable project
  ordering, newest-first sessions within each project, and metadata-only project
  archiving. Projects without displayable sessions stay hidden. `Ctrl+N` (`Cmd+N`
  on macOS) starts a new session
